...
export async function postToMastodon(content: string): Promise<void> {
  ...
  if (content.length > MAX_POST_LENGTH) {
    throw new Error(`Post is too long (${content.length} characters). Maximum is ${MAX_POST_LENGTH}.`);
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Mastodon API error (${response.status} ${response.statusText})${errorDetails}`);
    }
    ...
    throw new Error(`Failed to post to Mastodon: ${error instanceof Error ? error.message : String(error)}`);
  }
}
